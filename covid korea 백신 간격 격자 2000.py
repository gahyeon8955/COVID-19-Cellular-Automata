...
def seirpygame(cellCountX, cellCountY, universeTimeSeries):
    ...
    while mainloop:
        ...
        if cycletime > interval:             #if cycletime값이 interval값보다 클 때
            ...
            currentColor = BLACK
            for currentRow in range(cellCountY):    #Y축cellCount에 할당된 값을 range함수에 넣고 이 값의 범위 안의 값들을 하나하나 꺼내 currentRow에 넣어 for문 실행
                for currentColumn in range(cellCountX):     #X축cellCount에 할당된 값을 range함수에 넣고 이 값의 범위 안의 값들을 하나하나 꺼내 currentColumn에 넣어 for문 실행
...
def getNewState2D(currentRowNeighbours, upperRowNeighbours, lowerRowNeighbours): #2D의 새로운 상태를 얻는 함수(상부행, 중간행, 하부행 인자를 받아옴)
    ...
    # 줄 간격마다 백신
    for a in range(4, 101, 10):
        if currentColumn == a:
            newState = '4'
    
    for b in range(4, 101, 9):
        if currentRow == b:
            newState = '4'
    ...
    elif selfCharacter == '2':                                                             #selfCharacter가 '2'이면
...
InitInfected = 0.0
InitRecovered = 0.0
InitVariables = [InitSusceptibles, InitInfected, 0.0, 0.0, 0.0]                     #초기 상태 변수
...
for currentTimeStep in range(simulationIterations):
    ...
    RES.append([zeroCount, oneCount, twoCount, threeCount, currentTimeStep])        #RES에 각 상태의 셀 수와 currentTimeStep(전체 시간)을 저장
...
pl.subplot(2, 1, 1)
pl.plot(RES[:, 4], RES[:, 2], '-r', label='Infected')                                                        #RES[:,4]를 x축, RES[:,2]가 y축으로 갖는 solid line style의 붉은 색 그래프를 그리고 label(범례)에 Infected 저장
pl.plot(RES[:, 4], RES[:, 1], '-y', label='Exposed')                                                         #RES[:,4]를 x축, RES[:,1]가 y축으로 갖는 solid line style의 붉은 색 그래프를 그리고 label(범례)에 Exposed 저장
pl.plot(RES[:, 4], RES[:, 3], '-g', label='Recovered')                                                       #RES[:,4]를 x축, RES[:,3]가 y축으로 갖는 solid line style의 붉은 색 그래프를 그리고 label(범례)에 Recovered 저장
pl.plot(RES[:, 4], RES[:, 4], '-b', label='vaccine')                                                         #RES[:,4]를 x축, RES[:,3]가 y축으로 갖는 solid line style의 붉은 색 그래프를 그리고 label(범례)에 Recovered 저장
